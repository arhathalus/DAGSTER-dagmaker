/*************************
IPASIR glue for the MapleCOMSPS SAT solver (SAT Competition 2017; supply the
source yourself under maple/ -- see build_maple.sh).

MapleCOMSPS is a MiniSat/Glucose descendant (LRB + VSIDS branching), so this glue
is identical to glucose_glue.cc except for the `Minisat` namespace. We wrap the
core Solver (not SimpSolver) -- no variable elimination, hence fully incremental
with no need to freeze variables, matching Dagster's node-solver contract
(assume + solve + val + blocking-clause enumeration).

Build: see build_maple.sh (compiles MapleCOMSPS core + this glue -fPIC into
libipasirmaplecomsps.so, which Dagster's IpasirSolver dlopen's).
*************************/
...
const char* ipasir_signature() { return "maplecomsps-2017-ipasir (dagster)"; }
